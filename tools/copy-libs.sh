...
resolve_response_file() {
	...
	if [ -f "$rfile" ]; then
		...
		echo "Warning: response file not found: $rfile" >&2
	fi
}
...
if [ -d "$IDF_TOOLCHAIN_DIR" ]; then
	...
	done < <(tr ' ' '\n' < "$IDF_TOOLCHAIN_DIR/cflags" 2>/dev/null | grep -v '^$')
	while IFS= read -r rf_item; do
		if [[ $CPP_FLAGS != *"$rf_item"* && $PIOARDUINO_CC_FLAGS != *"$rf_item"* && $PIOARDUINO_CXX_FLAGS != *"$rf_item"* ]]; then
			CPP_FLAGS+="$rf_item "
		fi
	done < <(tr ' ' '\n' < "$IDF_TOOLCHAIN_DIR/cxxflags" 2>/dev/null | grep -v '^$')
	...
	done < <(tr ' ' '\n' < "$IDF_TOOLCHAIN_DIR/ldflags" 2>/dev/null | grep -v '^$')
	...
	if [ -f "$IDF_TOOLCHAIN_DIR/cflags" ] && [ -f "$IDF_TOOLCHAIN_DIR/cxxflags" ]; then
		...
		done < <(tr ' ' '\n' < "$IDF_TOOLCHAIN_DIR/cflags" | grep -v '^$')
		while IFS= read -r rf_item; do
		...
		done < <(tr ' ' '\n' < "$IDF_TOOLCHAIN_DIR/cxxflags" | grep -v '^$')
	fi
fi
